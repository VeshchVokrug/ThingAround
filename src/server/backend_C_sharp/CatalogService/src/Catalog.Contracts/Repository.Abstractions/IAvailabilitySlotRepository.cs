...
public interface IAvailabilitySlotRepository
{
    ...
    Task CreateRangeAsync(Guid listingId, int price, IEnumerable<DateOnly> dates, CancellationToken ct = default);
    ...
    Task<int> RemoveRangeAsync(Guid listingId, IEnumerable<DateOnly> dates, CancellationToken ct = default);
}
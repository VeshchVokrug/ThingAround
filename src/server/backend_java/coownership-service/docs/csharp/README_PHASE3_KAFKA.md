...
3. Если найден активный период по `rentalListingId`, все слоты периода в диапазоне дат переводятся в `BOOKED`.
4. `totalIncome` периода увеличивается на `totalPrice`.
5. Если активного периода нет, сообщение не меняет состояние и считается обработанным.

## Ошибки и ограничения
- Некорректный JSON приводит к `IllegalArgumentException` с сообщением о некорректном формате Kafka-события.
- Невалидные UUID или неверный формат дат не проходят парсинг и также приводят к ошибке на этапе обработки сообщения.
...
- `src/test/java/ru/veshvokrug/coownership/input/kafka/CoownershipInboundKafkaConsumerTest.java`
  ...
  - проверяет отказ на некорректном JSON

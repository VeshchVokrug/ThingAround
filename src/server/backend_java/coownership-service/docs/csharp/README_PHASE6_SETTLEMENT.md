...
Примечания:
- Числа с плавающей точкой в поле `amount` имеют два знака после точки (процентная доля округляется с RoundingMode.HALF_UP).
- `totalIncome` — число с двумя знаками (например, 1000.00). При сериализации в JSON тип `BigDecimal` представляется как число.
- `settlements` является списком строк, порядок владельцев соответствует сохранённой итерации (в коде используется LinkedHashMap).
